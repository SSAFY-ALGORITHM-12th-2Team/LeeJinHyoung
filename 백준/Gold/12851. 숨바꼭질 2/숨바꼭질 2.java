...
//		결국은 순간이동으로 절댓갑스이 차가 가장 적은 곳으로 이동하는 게 핵심
		...
		while (!q.isEmpty()) {
			...
			if (cur.position * 2 < position.length && position[cur.position * 2] >= cur.time + 1) {
				q.offer(new move(cur.time + 1, cur.position * 2));
				position[cur.position * 2] = Math.min(position[cur.position] + 1, position[cur.position * 2]);
			}
		}
